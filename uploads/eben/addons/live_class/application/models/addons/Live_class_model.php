...
class Live_class_model extends CI_Model {
	...
	public function store() {
		...
		if ($_FILES['attachment']['name']) {
			if (!file_exists($this->config->item('asset_url').'/live_class')) {
				mkdir($this->config->item('asset_url').'/live_class', 0777, true);
			}
			$file_ext = pathinfo($_FILES['attachment']['name'], PATHINFO_EXTENSION);
			$data['attachment'] = md5(rand(10000000, 20000000)).'.'.$file_ext;
			move_uploaded_file($_FILES['attachment']['tmp_name'], $this->config->item('asset_url').'/live_class/'.$data['attachment']);
		}
	...
	public function get_live_class_settings() {
		$settings_data = $this->db->get_where('live_class_settings',['school_id'=>$this->school_id])->row_array();
	...
	public function update_settings() {
		...
		$previous_data = $this->db->get_where('live_class_settings', ['school_id' => $this->school_id]);
