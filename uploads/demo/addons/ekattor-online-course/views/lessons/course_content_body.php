<div class="col-lg-9  order-md-1 course_col" id = "video_player_area">
    <div class="text-center">
        ...
        $lesson_thumbnail_url = $this->config->item('asset_url').'/course_thumbnail/'.$this->lms_model->get_course_by_id($course_id)['thumbnail'];
        if (file_exists($lesson_thumbnail_url)){
            $lesson_thumbnail_url = base_url().$lesson_thumbnail_url;
        } else {
            $lesson_thumbnail_url = base_url().$this->config->item('asset_url').'/course_thumbnail/placeholder.png';
        }
        ...
        <?php else: ?>
            <div class="mt-5">
                <a href="<?php echo base_url().$this->config->item('asset_url').'/lesson_files/'.$lesson_details['attachment']; ?>" class="btn btn-info text-white" download>
