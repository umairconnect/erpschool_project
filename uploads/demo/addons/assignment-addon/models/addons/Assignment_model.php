	...
	public function save_answers($question_id = "", $assignment_id = ""){
		...
		if($question_type == 'file'){
			if(!empty($_FILES['question_answer']['name'])){
	            ...
	            move_uploaded_file($_FILES['question_answer']['tmp_name'], $this->config->item('asset_url').'/assignment_files/' . $data['answer']);
            }else{
		...
		if($question_answer->num_rows() > 0 && $error_message == null){
			$previews_uploaded_file = $this->config->item('asset_url').'/assignment_files/'.$question_answer->row('answer');
