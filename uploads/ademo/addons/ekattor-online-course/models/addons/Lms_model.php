...
class Lms_model extends CI_Model {
  ...
  // constructor
    ...
    public function delete_course($course_id){
        ...
        if(file_exists($this->config->item('asset_url').'/course_thumbnail/'.$course['thumbnail']))
        unlink($this->config->item('asset_url').'/course_thumbnail/'.$course['thumbnail']);
    ...
    public function course_add(){
        ...
        move_uploaded_file($_FILES['course_thumbnail']['tmp_name'], $this->config->item('asset_url').'/course_thumbnail/'.$data['thumbnail']);
    }
    ...
    public function course_edit($course_id){
        ...
        if($_FILES['course_thumbnail']['tmp_name']){
            unlink($this->config->item('asset_url').'/course_thumbnail/'.html_escape($this->input->post('current_thumbnail')));
            $data['thumbnail'] = rand().'.jpg';
            move_uploaded_file($_FILES['course_thumbnail']['tmp_name'], $this->config->item('asset_url').'/course_thumbnail/'.$data['thumbnail']);
        }
    ...
    public function add_lesson(){
        ...
        if ($lesson_type == 'video') {
            ...
            if ($_FILES['attachment']['name'] == "") {
                ...
                if (!file_exists($this->config->item('asset_url').'/lesson_files')) {
                    mkdir($this->config->item('asset_url').'/lesson_files', 0777, true);
                }
                move_uploaded_file($_FILES['attachment']['tmp_name'], $this->config->item('asset_url').'/lesson_files/' . $uploadable_file);
            }
        }
        ...
        if ($_FILES['thumbnail']['name'] != "") {
            if (!file_exists($this->config->item('asset_url').'/thumbnails/lesson_thumbnails')) {
                mkdir($this->config->item('asset_url').'/thumbnails/lesson_thumbnails', 0777, true);
            }
            move_uploaded_file($_FILES['thumbnail']['tmp_name'], $this->config->item('asset_url').'/thumbnails/lesson_thumbnails/' . $inserted_id . '.jpg');
        }
    }
        ...
        if ($lesson_type == 'video') {
            ...
            if ($lesson_provider == 'youtube' || $lesson_provider == 'vimeo') {
                ...
                if ($_FILES['thumbnail']['name'] != "") {
                    ...
                    move_uploaded_file($_FILES['thumbnail']['tmp_name'], $this->config->item('asset_url').'/thumbnails/lesson_thumbnails/' . $lesson_id . '.jpg');
                }
            } else {
                $this->session->set_flashdata('error_message', get_phrase('invalid_lesson_provider'));
                redirect(site_url(strtolower($this->session->userdata('role')) . '/course_form/course_edit/' . $data['course_id']), 'refresh');
            ...
            if ($_FILES['attachment']['name'] != "") {
                ...
                if ($previous_data['attachment'] != "") {
                    unlink($this->config->item('asset_url').'/lesson_files/' . $previous_data['attachment']);
                }
