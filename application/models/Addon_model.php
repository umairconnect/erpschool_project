...
class Addon_model extends CI_Model {
	...
	public function install_addon() {
		...
		if (!empty($zipped_file_name)) {
			...
			$dir = $this->config->item('asset_url').'/addons';
			if (!is_dir($dir))
				mkdir($dir, 0777, true);
			...
			$res = $zip->open($path);
			if ($res === TRUE) {
				$zip->extractTo($this->config->item('asset_url').'/addons');
				$zip->close();
				unlink($path);
			}

			$unzipped_file_name = substr($zipped_file_name, 0, -4);
			$config_str = file_get_contents($this->config->item('asset_url').'/addons/' . $unzipped_file_name . '/config.json');
