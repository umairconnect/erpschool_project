...
class Frontend_model extends CI_Model {
  ...
  function add_news() {
    ...
    if ($_FILES['news_image']['name'] != '') {
      ...
      move_uploaded_file($_FILES['news_image']['tmp_name'], $this->config->item('asset_url').'/frontend/news_image/'. $_FILES['news_image']['name']);
    }
  ...
  function delete_news($news_id) {
    ...
    if ($news_image != NULL) {
      if (file_exists($this->config->item('asset_url').'/frontend/news_image/'. $news_image)) {
        unlink($this->config->item('asset_url').'/frontend/news_image/'. $news_image);
      }
    }
    // delete the db entry
  ...
  function add_frontend_gallery() {
    ...
    if ($_FILES['cover_image']['name'] != '') {
      ...
      move_uploaded_file($_FILES['cover_image']['tmp_name'], $this->config->item('asset_url').'/images/gallery_cover/'. $data['image']);
    }
  ...
  public function upload_gallery_photo($gallery_id) {
    if (isset($_FILES['gallery_photo']) && !empty($_FILES['gallery_photo']['name'])) {
			...
			move_uploaded_file($_FILES['gallery_photo']['tmp_name'], $this->config->item('asset_url').'/images/gallery_images/'.$data['image']);
  ...
  function add_gallery_images($gallery_id) {
    ...
    for ($i=0; $i < $number_of_images; $i++) {
      if ($files['gallery_images']['name'][$i] != '') {
        move_uploaded_file($files['gallery_images']['tmp_name'][$i], $this->config->item('asset_url').'/frontend/gallery_images/'. $files['gallery_images']['name'][$i]);
        $data['frontend_gallery_id']  = $gallery_id;
  ...
  function delete_gallery_image($gallery_image_id) {
    ...
    if (file_exists($this->config->item('asset_url').'/frontend/gallery_images/'.$image)) {
      unlink($this->config->item('asset_url').'/frontend/gallery_images/'.$image);
    }
  ...
  public function get_gallery_image($image = "") {
    if (file_exists($this->config->item('asset_url').'/images/gallery_images/'.$image))
    return base_url().$this->config->item('asset_url').'/images/gallery_images/'.$image;
    else
    return base_url().$this->config->item('asset_url').'/images/gallery_images/placeholder.png';
  }
  ...
  function update_about_us() {
    ...
    if ($_FILES['about_us_image']['name'] != '') {
      move_uploaded_file($_FILES['about_us_image']['tmp_name'], $this->config->item('asset_url').'/images/about_us/about-us.jpg');
    }
  ...
  function update_homepage_slider() {
    ...
    for ($i=0; $i < 3; $i++) {
      ...
      if ($_FILES['slider_image_'.$i]['name'] != '') {
        ...
        move_uploaded_file($_FILES['slider_image_'.$i]['tmp_name'], $this->config->item('asset_url').'/images/slider/'. $data['image']);
      } else {
        $data['image']  = $image;
      }
  ...
  function update_frontend_general_settings() {
    ...
    if ($_FILES['header_logo']['name'] != '') {
      move_uploaded_file($_FILES['header_logo']['tmp_name'], $this->config->item('asset_url').'/system/logo/header-logo.png');
    }

    if ($_FILES['footer_logo']['name'] != '') {
      move_uploaded_file($_FILES['footer_logo']['tmp_name'], $this->config->item('asset_url').'/system/logo/footer-logo.png');
    }
  ...
  public function get_header_logo() {
    return base_url($this->config->item('asset_url').'/system/logo/header-logo.png');
  }
  // GET FOOTER LOGO
  public function get_footer_logo() {
    return base_url($this->config->item('asset_url').'/system/logo/footer-logo.png');
  }

  //GET ABOUT IMAGE
  public function get_about_image() {
    return base_url($this->config->item('asset_url').'/images/about_us/about-us.jpg');
  }

  //GET SLIDER IMAGE
  public function get_slider_image($image) {
    return base_url($this->config->item('asset_url').'/images/slider/'.$image);
  }

  public function remove_image($type = "", $photo = "") {
		$path = $this->config->item('asset_url').'/images/'.$type.'/'.$photo;
		if(file_exists($path)){
			unlink($path);
		}
	}
