...
class Superadmin extends CI_Controller {
  ...
  public function manage_class($param1 = '', $param2 = '', $param3 = ''){
    ...
    if($param1 == 'get'){
      $response = $this->crud_model->get_class($param2);
      echo $response;
    }

    if($param1 == 'get_areas'){
      $response = $this->crud_model->get_areas($param2);
      echo $response;
    }    

    if($param1 == 'get_steps'){
      $response = $this->crud_model->get_steps($param2);
      echo $response;
    } 
  ...
  public function register_class($param1 = "", $param2 = "") {
    if ($param1 == 'insert') {
      $response = $this->crud_model->register_new_class();
      echo $response;
    }
    ...
    if(empty($param1)){
      $page_data['folder_name'] = 'class';
      $page_data['page_title']  = 'register class';
      $page_data['page_name'] = 'signup';
      $this->load->view('backend/index',$page_data);
    }
  }

  public function director_signup($param1 = "", $param2 = "") {
    if ($param1 == 'insert') {
      $response = $this->user_model->director_signup();
      echo $response;
    }

    if ($param1 == 'search') {
      $page_data['folder_name'] = 'school_director';
      $page_data['page_title']  = 'School Diretor';
      $page_data['page_name'] = 'search';
      $this->load->view('backend/index',$page_data);
    }    

    if ($param1 == 'link') {
      $response = $this->user_model->director_link();
      echo $response;
    }    
    // showing the System Settings file
    if(empty($param1)){
      $page_data['folder_name'] = 'school_director';
      $page_data['page_title']  = 'School Diretor';
      $page_data['page_name'] = 'director_signup';
      $this->load->view('backend/index',$page_data);
    }
  }  
  //	SECTION STARTED
  ...
  public function teacher($param1 = '', $param2 = '', $param3 = ''){
    ...
    if ($param1 == 'link') {
      $response = $this->user_model->teacher_link();
      echo $response;
    }        
    if($param1 == 'signup'){
      $page_data['folder_name'] = 'teacher';
    ...
    if ($param1 == 'insert') {
      $response = $this->user_model->teacher_signup();
      echo $response;
    }
  ...
  public function student($param1 = '', $param2 = '', $param3 = '', $param4 = '', $param5 = ''){
    ...
    if ($param1 == 'insert') {
      $response = $this->user_model->student_signup();
      echo $response;
    }    
    if(empty($param1)){
  ...
  public function description($param1 = "", $param2 = "") {
    if ($param1 == 'update') {
      $response = $this->settings_model->insert_new_description();
      // echo $response;
      
        $page_data['folder_name'] = 'settings';
        $page_data['page_title']  = 'organization';
        $page_data['settings_type'] = 'organization';
        $this->load->view('backend/index', $page_data);
    ...
    if(empty($param1)){
      $page_data['folder_name'] = 'settings';
      $page_data['page_title']  = 'description';
      $page_data['settings_type'] = 'description';
      $this->load->view('backend/index', $page_data);
    }
  }
  ...
  public function organization($param1 = "", $param2 = "") {
    if ($param1 == 'update') {
      $response = $this->settings_model->insert_new_organization();
      echo $response;
    }
  ...
  // ABOUT APPLICATION ENDS
    public function exportdb(){
	  $page_data['folder_name'] = 'Export';
      $page_data['page_title']  = 'Export';
	  $this->load->view('backend/index', $page_data);
  }
  public function exporttable($param1 = "",$param2 = ""){
	 if ($param1 == 'url') {
      $type = $this->input->post('type');
      $table = $this->input->post('table');
     
      echo route('exporttable/'.$type.'/'.$table);
    }
	if($param1 == 'csv'){
  

			$invoices = $this->crud_model->gettabledata($param2)->result_array();
	  
            header("Content-type: application/csv");
            header("Content-Disposition: attachment; filename=\".$param2.".".csv\"");
            header("Pragma: no-cache");
            header("Expires: 0");

            $handle = fopen('php://output', 'w');

            foreach ($invoices as $data_array) {
	...
	if($param1 == 'csv1'){
      ...
      $csv_file = fopen("assets/csv_file/users.csv", "w");
      $header = array('Invoice-no', 'Student', 'Class', 'Invoice-Title', 'Total-Amount', 'Paid-Amount', 'Creation-Date', 'Payment-Date', 'Status');
      fputcsv($csv_file, $header);
      foreach ($invoices as $invoice) {

        $lines = array(sprintf('%08d', $invoice['id']), $invoice['name'], $invoice['email'], $invoice['role'], $invoice['address'], $invoice['phone'], $invoice['birthday'], $invoice['gender']);
        fputcsv($csv_file, $lines);
      }


      $fileName = 'users.csv';
      $this->download_file('assets/csv_file/users.csv', $fileName);
    }
